...
/**
 * Analyzes packages using npms.io API
 * @param {Object} projectData - Project data from scanner
...
export async function analyzePackages(projectData, options) {
  ...
  try {
    ...
    // Fetch package data in batches to avoid API limits
    const batchSize = 50;
    const batches = [];
    ...
    // Sort packages by score (lowest first)
